...
// }
    ...
    function saveQuizData() {
        ...
        quizData.types.forEach(type => {
            ...
            if (typeCount[lowerType] !== undefined) {
                typeCount[lowerType]++;
            }
        });
        
        const total = quizData.types.length || 1;
        const ratios = {
            vata: ((typeCount.vata / total) * 100).toFixed(2),
            pitta: ((typeCount.pitta / total) * 100).toFixed(2),
            kapha: ((typeCount.kapha / total) * 100).toFixed(2)
        };
        
        const formData = new FormData();
        formData.append('name', quizData.name);
        formData.append('age_ranges', quizData.age_ranges.join(','));
        formData.append('email', quizData.email);
        formData.append('phone', quizData.phone);
        formData.append('body_type', bodyType);
        formData.append('body_type_ratios', JSON.stringify(ratios));
        formData.append('selected_choices', quizData.selections.join(','));
    ...
    function selectOption(questionId, choiceId) {
        ...
        // Collect body type from data-type attribute
        const type = selectedBtn.getAttribute('data-type');
        if (type && type !== 'neutral' && type !== '') {
            quizData.types.push(type);
        }
    }

    // Function to determine body type with proper counting
    function determineBodyType() {
        const typeCount = { vata: 0, pitta: 0, kapha: 0 };
        
        // Count each body type occurrence
        quizData.types.forEach(type => {
            const lowerType = type.toLowerCase().trim();
            if (typeCount[lowerType] !== undefined) {
                typeCount[lowerType]++;
        ...
        // Find the body type with maximum count
        let maxType = 'vata';
        let maxCount = 0;
